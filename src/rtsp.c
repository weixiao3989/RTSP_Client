...
// #Chris 20230103 Added.
void substring(char *dst,char *src,int start,int len)
{  
	char *p=dst;  
	char *q=src;  
...
int32_t RtspDescribeCommand(RtspSession *sess)
{
    ...
    para_1 = strstr(buf, "sprop-parameter-sets=");
    fprintf(stderr, "\n### #Chris Log -> para_1 : %s", para_1);
    strncpy(sps, para_1+21, 32);
    fprintf(stderr, "\n### #Chris Log -> sps : %s", sps);

    para_2 = strstr(buf, "sprop-parameter-sets=");
    fprintf(stderr, "\n### #Chris Log -> para_2 : %s", para_2);
    strncpy(pps, para_2+54, 8);
    fprintf(stderr, "\n### #Chris Log -> pps : %s", pps);
